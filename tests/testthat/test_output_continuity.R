...
test_that("Ensure continuity of stress test outputs against snapshot", {
  ...
  current_npv <- st_results_current$npv_results %>%
    select(-c(run_id)) %>%
    arrange_all()
  snapshot_npv <- snapshot_results$npv_results %>%
    select(-c(run_id)) %>%
    arrange_all()
  ...
  current_pd <- st_results_current$pd_results %>%
    select(-c(run_id)) %>%
    arrange_all()
  snapshot_pd <- snapshot_results$pd_results %>%
    select(-c(run_id)) %>%
    arrange_all()
  ...
  current_trajectories <- st_results_current$company_trajectories %>%
    select(-c(run_id)) %>%
    arrange_all()
  snapshot_trajectories <- snapshot_results$company_trajectories %>%
    select(-c(run_id)) %>%
    arrange_all()
