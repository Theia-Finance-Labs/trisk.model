...
```{r}
if ("trisk.model" %in% installed.packages()){
  if (requireNamespace("package_name", quietly = TRUE)) {
    detach("package:trisk.model", unload=TRUE)
  }
  remove.packages("trisk.model")
}
...
```{r message=FALSE}
st_results_branch <- trisk.model::run_trisk(input_path = ST_INPUTS_FOLDER, output_path = tempdir(), return_results = TRUE)
```
...
detach("package:trisk.model", unload=TRUE)
remove.packages("trisk.model")
devtools::install_github("2DegreesInvesting/trisk.model", upgrade = "never")
```
...
```{r message=FALSE}
st_results_master<- trisk.model::run_trisk(input_path = ST_INPUTS_FOLDER, output_path = tempdir(), return_results = TRUE)
```
# Compare results
